    ...
    <div className="min-h-screen flex items-center justify-center bg-[#0a0a0b] p-4">
      <div className="w-full max-w-md">
        ...
        <div className="bg-[#111113] border border-[#252529] rounded-2xl p-6 space-y-4">
          {magicLinkSent ? (
            <div className="text-center py-6">
              ...
              <button onClick={() => setMagicLinkSent(false)} className="mt-3 text-xs text-[#f59e0b] hover:underline">Try another method</button>
            </div>
          ) : (
            <>
              <div className="flex gap-1 p-1 bg-[#0a0a0b] rounded-lg">
                <button onClick={() => setMode('magic')} className={`flex-1 py-2 text-xs rounded-md transition ${mode === 'magic' ? 'bg-[#f59e0b] text-[#0a0a0b] font-semibold' : 'text-[#6b6b73]'}`}>Magic Link</button>
                <button onClick={() => setMode('password')} className={`flex-1 py-2 text-xs rounded-md transition ${mode === 'password' ? 'bg-[#f59e0b] text-[#0a0a0b] font-semibold' : 'text-[#6b6b73]'}`}>Password</button>
              </div>

              <form onSubmit={mode === 'password' ? handleEmailLogin : handleMagicLink} className="space-y-3">
                <div>
        ...
        <p className="text-center text-[#45454c] text-xs mt-4">
          No account? <Link href="/register" className="text-[#f59e0b] hover:underline">Create one</Link>
        </p>
      </div>
    </div>
  )
